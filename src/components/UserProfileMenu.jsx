...
const UserProfileMenu = () => {
  ...
  const getUserData = async () => {
    const { data } = await axios("https://jobme-server-ntj9.onrender.com/api/v1/user", {headers:{Authorization: `Bearer ${token}`}});
    setUser(data.user)
    setIsLoading(false)
  };
  getUserData()
      ...
      {!isLoading ? (
        <div className="d-flex gap-2 align-items-center border border-dark border-start-0 border-end-0 py-1 mt-2">
          ...
          <div>
            ...
            <small style={{ fontSize: "12px" }}>{user?.email || "email"}</small>
          </div>
        </div>
      ) : (
        <Loader height="30px" />
